# GitHub Copilot CLI：CLI 參數整理（Table 版）

> 本節以 **CLI 啟動時可用參數** 為核心, 整理成表格, 方便查閱與工程實務使用。
...
| `-p (Get-Content FILE)`<br/>`-p $(cat FILE)` | `copilot -p (Get-Content prompt.txt)`<br/>`copilot -p $(cat prompt.txt)` | 引用 file 內容當作 prompt, 不進互動模式 | 中            |  |
