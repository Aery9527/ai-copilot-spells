...
一鍵在本機安裝 Claude Code 的自訂 status line，包含即時執行狀態顯示（◉ 執行中 / ◉ 等待指示）：

- 將 [`statusline-command.sh`](../cli-agents/claude-code/statusline-command.sh)（repo 內 git tracked）複製到 `~/.claude/statusline-command.sh`
- 將 [`cli-agents/claude-code/hooks/`](../cli-agents/claude-code/hooks/) 下的所有 `.sh` 複製到 `~/.claude/hooks/`
- 在 `~/.claude/settings.json` 寫入 `statusLine` key
- 在 `~/.claude/settings.json` 注入 `UserPromptSubmit`、`PreToolUse`、`Stop` 三個 lifecycle hooks，用於即時更新狀態暫存檔
...
| Hook 事件 | 觸發時機 | 寫入狀態 |
|-----------|----------|----------|
| `UserPromptSubmit` | 使用者送出 prompt | `running` |
| `PreToolUse` | 工具執行前 | `running` |
| `Stop` | Claude 回應完成 | `idle` |

Hook 腳本從 stdin 讀取 `session_id`，寫入 `$TMPDIR/claude_state_<session_id>`。StatusLine 腳本讀取此檔決定顯示狀態。

#### 參數

| 參數 | 型別 | 說明 |
|------|------|------|
| `-Force` | `switch` | 強制覆寫所有目標 `.sh`，即使目標已存在且與來源相同 |

#### 它實際在做什麼

1. 以 `$PSScriptRoot\..` 推算 repo 根目錄。
2. 確認 `~/.claude/` 及 `~/.claude/hooks/` 目錄存在，不存在則建立。
3. 比對來源與目標 SHA-256；若不同（或 `-Force`）則複製 `statusline-command.sh` 及 `hooks/*.sh`。
4. 讀取 `~/.claude/settings.json`（不存在則初始化為空物件）。
5. 若 `statusLine` key 不存在則新增，已存在則更新，其他欄位原封不動。
6. 逐一檢查 `UserPromptSubmit`、`PreToolUse`、`Stop` 三個 hook 事件：若指令不存在則新增，已存在則略過（不重複也不覆蓋現有 hooks）。
7. 將修改後的 JSON 寫回 `~/.claude/settings.json`（UTF-8，無 BOM）。
...
- Hook 注入採「只新增，不覆蓋」策略：相同 `command` 字串已存在則略過，不會重複新增；但也不移除使用者已有的其他 hooks。
...
```text
Copied : ...\cli-agents\claude-code\statusline-command.sh -> C:\Users\..\.claude\statusline-command.sh
Created: C:\Users\..\.claude\hooks
Copied : ...\hooks\state-idle.sh -> C:\Users\..\.claude\hooks\state-idle.sh
Copied : ...\hooks\state-running.sh -> C:\Users\..\.claude\hooks\state-running.sh
Updated: statusLine in C:\Users\..\.claude\settings.json
Updated: hooks in C:\Users\..\.claude\settings.json
